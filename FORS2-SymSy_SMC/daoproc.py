...
iraf.datapars.setParam('sigma','11.8')	
iraf.datapars.setParam('datamin','255.11')	
iraf.datapars.setParam('datamax','25000')

Nimages = 3 				# number of images used
iraf.datapars.setParam('readnoise',math.sqrt(Nimages)*rdnoise)	
iraf.datapars.setParam('epadu',gain*Nimages)

iraf.datapars.setParam('itime','179.9931')

maxpsfstars = 50			# number of stars for psf model

# UP TO HERE <<< 

# DO NOT EDIT ANY FURTHER (unless you understand what you are doing)
...
#	iraf.phot(image='F1_Rc.fits',coords='stars2check.coo',output='stars2check-orig.mag', verify='no')	# on subtracted image
#	iraf.phot(image='F1_Rc.sub.3.fits',coords='stars2check.coo',output='stars2check-sub.mag', verify='no')	# on subtracted image
	tpe = time.time()
...
#	sys.exit('exiting NOW!')
	...
	if not os.path.exists(flnm+'.psf.fits'):
		...
		os.system("/home/gmaravel/Softwr/ds9/ds9 "+flnm+'.sub.0.fits'+" -regions "+flnm+".pst.1.reg"+" &")
...
#		iraf.psf(image=flnm+'.sub.0.fits',photfile=flnm+'.sub.0.mag',pstfile=flnm+'.pst.1',psfimage=flnm+'.psf.fits',opstfile=flnm+'.pst.2',groupfile=flnm+'.grf.psg',interactive="no",showplots="no",verify="no")
...
#	sys.exit('EXITING ...psf module - rerun!')
...
print "\nThe DaoProcess has been executed, me Lord!"
